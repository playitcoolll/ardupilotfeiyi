...
{   
    //servo_in是一个deg*100的舵偏角值
    //simulink模型中的混控逻辑为：servo_in * （1-k） + 飞手的舵面偏转输入 * k
    //k_max的值随速度sp变化
    const AP_AHRS &_ahrs = AP::ahrs();
    float sp;
    float k_max;
    if (!_ahrs.airspeed_estimate(sp)) {
        sp = 0;
    }
    if(sp <= 30){
        k_max = -(float)1/30 *(sp - 30.0f);
    }else{
        k_max = 0;
    }
    //缺点:与速度有关，如果拉杆姿态增大了，速度下降 那么飞手控制权增大，导致相同拉杆量使飞机的姿态变化增大，进一步导致飞机速度的变化，进而影响整个过程
    //注意 首先需要确定输入的pwm值和输出的pwm值的关系 还有输出的pwm值与舵偏角的关系
    //按控制系统的来说 应该为：1100-1900对应-45 - 45度的舵偏角
    ...
    float ch_inf = (float)(control_in - 0);
    ch_inf = fabsf(ch_inf);
    ch_inf = ((4500.0f - ch_inf) / 4500.0f);



    float ch_inf_feikong = ch_inf;
    float ch_inf_feishou = 1 - ch_inf;

    // float ch_inf_feikong = 0.7;
    // float ch_inf_feishou = 1 - 0.7;

    constrain_float(ch_inf_feikong, 1-k_max, 1);
    constrain_float(ch_inf_feishou, 0, k_max);

    // hal.console -> printf(" %f\n",ch_inf_feikong);
    // hal.console -> printf(" %f\n",ch_inf_feishou);

    float servo_out = servo_in;
    servo_out *= ch_inf_feikong;
    servo_out += (control_in * ch_inf_feishou);
    // tm = AP_HAL::millis();
