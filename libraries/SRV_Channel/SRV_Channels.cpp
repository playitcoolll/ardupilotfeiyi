...
void SRV_Channels::calc_pwm(void)
{
    ...
    for (uint8_t i=0; i<NUM_SERVO_CHANNELS; i++) {
        // override_counter[i]--; 的作用是在每次循环中递减 override_counter 数组中对应通道的计数值，用于跟踪通道被锁定的剩余次数。
        //锁定计数递减到 0 时，通道将被解锁并恢复正常操作。这种机制通常用于暂时禁用某些通道的输出，例如在进行某些校准或特殊操作时
        ...
        //如果通道有有效的功能（即 valid_function() 返回 true），则计算该通道的PWM值，使用对应功能的 output_scaled 进行计算。
