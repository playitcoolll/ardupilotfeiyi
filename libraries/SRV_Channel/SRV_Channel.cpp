...
{
    //如果输出只能为0，则pwm为中立位 一般舵面的high_out = 4500 
    ...
    //     scaled_value = -scaled_value;
    // }
    // scaled_value = constrain_float(scaled_value, -my_high_out, my_high_out);
    // if (scaled_value > 0) {
    //     return servo_trim + uint16_t( (scaled_value * (float)(servo_max - servo_trim)) / (float)my_high_out);
    // } else {
    //     return servo_trim - uint16_t( (-scaled_value * (float)(servo_trim - servo_min)) / (float)my_high_out);
    // }
}
...
uint16_t SRV_Channel::pwm_from_scaled_value(float scaled_value) const
{
    if (type_angle) {
        ...
        //     last_tm = tm;
        // }
        //这个地方打印的是舵面通道的值，rcin输入的1000-1500-2000经过俯仰或滚转控制后，控制律输出的是角度-30到30（*100）的scale 然后再转为pwm
        //舵面调节：例如15度舵偏角，scale值为1500，转换位pwm后的值无所谓，但需要这个值对应的物理舵面偏角为15度（如果舵机pwm与连杆角度有关的话，pwm就有所谓了）
        ...
        //     last_tm = tm;
        // }
        //这个地方打印的是油门通道的值，rcin输入的1000-1500-2000对应0-100的scale 然后再转为pwm
