...
void JSON::output_servos(const struct sitl_input &input)
{
    ...
    if (SRV_Channels::have_32_channels()) {
      ...
      for (uint8_t i=0; i<16; i++) {
        ...
        //测试输入  simulink中的输入等于这个地方的值
        // sitl_tm = AP_HAL::millis();
        // if(sitl_tm - sitl_last_tm >1000){
        //     uint16_t testpwm;
        //     if (SRV_Channels::get_output_pwm(SRV_Channel::k_elevator,testpwm)){
        //         hal.console -> printf("%d\n",testpwm);
        //     }
        //     // hal.console -> printf("%d\n",pkt.pwm[0]);//注意 c中向量索引从0开始 （长度为16的向量 索引为0-15）
        //     // hal.console -> printf("%d\n",SRV_Channels::channels[0]);
        //     sitl_last_tm = sitl_tm;
        // }
    ...
    //simulink连接的时候 走的是这个json文件
    // sitl_tm = AP_HAL::millis();
    // if(sitl_tm - sitl_last_tm >1000){
    //     hal.console -> printf("1111111111111111111111111\n");
    //     sitl_last_tm = sitl_tm;
    // }
