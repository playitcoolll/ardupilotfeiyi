...
{
    //手动模式?
    float aileron  = filtered_servo_angle(input, 0); //rc1
    float elevator = filtered_servo_angle(input, 1); //rc2
    float rudder   = filtered_servo_angle(input, 3); //rc4
    bool launch_triggered = input.servos[6] > 1700;

    //simulink中的仿真不在这个地方 但运行自带的arduplane就运行这个地方了
