...
void Plane::dspoiler_update(void)
{
    ...
    float dspoiler_outer_left = 0;
    float dspoiler_outer_right = 0;

    float dspoiler_inner_left = elevon_left;
    float dspoiler_inner_right = elevon_right;
    ...
    if (rudder > 0) {
        ...
        dspoiler_outer_right = constrain_float(dspoiler_outer_right + rudder, 0, 4500);
        dspoiler_inner_right = constrain_float(dspoiler_inner_right + 0.5 * rudder, -4500, 4500);
    } else {
        // apply rudder to left wing
        dspoiler_outer_left = constrain_float(dspoiler_outer_left - rudder, 0, 4500);
        dspoiler_inner_left = constrain_float(dspoiler_inner_left - 0.5 * rudder, -4500, 4500);
    }
...
void Plane::servos_output(void)
{
    ...
    //此处打印这个值没用，因为他还是最终计算得到的scale值（虽然是自己设定的期望的舵偏角），并不是pwm值
    // tm = AP_HAL::millis();
    // if(tm - last_tm >5000){
    //     hal.console -> printf("%f2222222222222222\n",SRV_Channels::get_output_scaled(SRV_Channel::k_aileron));
    //     last_tm = tm;
    // }
